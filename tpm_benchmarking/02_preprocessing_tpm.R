...
genes_of_interest <- c('ENSG00000067715', 'ENSG00000198793', 'ENSG00000118689', 'ENSG00000096717', 'ENSG00000132639', 'ENSG00000106631', 'ENSG00000197616', 'ENSG00000155657', 'ENSG00000092054', 'ENSG00000163631', 'ENSG00000110245', 'ENSG00000167768', 'ENSG00000186395','ENSG00000141510', 'ENSG00000077463', 'ENSG00000130203')
...
# dat_filtered <- dat_filtered[, apply(dat_filtered, 2, var) != 0, drop = FALSE]  
...
dir.create('data/processed/expression/tpm_all_tis/', recursive = TRUE, showWarnings = FALSE)  

# Save the filtered data and metadata  
sapply(names(dat_filtered_tissues), function(nm) {  
  saveRDS(dat_filtered_tissues[[nm]], file.path('data/processed/expression/tpm_all_tis/', paste0(nm, '.rds')))  
})  

saveRDS(attphe, './data/processed/attphe_tpm_all_tis.rds')  
